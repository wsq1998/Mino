...
打开 `dist/Mio-1.3.0-arm64.dmg`，把 Mio 拖进 Applications 即可。
...
| 右键菜单 | 置顶开关 / 透明度 / 退出 |
| 召唤 / 隐藏 | 全局快捷键 `⌥ + Space` |
| 看视频 / 演示时自动隐身 | 前台是播放器 / 演示 / 会议类应用时，球体自动淡化到 12% |
...
| 功能 | 交互 |
|------|------|
| 开机自启动 | 开关。**仅打包版可写**，开发模式自动置灰并提示 |
| 整点报时 | 开关 + 报时时段 + 是否同时发系统通知 |
| 健康提醒 | 总开关 + 久坐 / 喝水 / 护眼各自开关 + 免打扰时段 |
| 智能隐身 | 开关 + 已收录应用清单 + `⌥H` 快捷键说明 |
...
./pack.sh --dmg          # 组装 + 重新生成 dist/Mio-1.3.0-arm64.dmg
```
